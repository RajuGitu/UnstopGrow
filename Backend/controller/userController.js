...
const loginUserController = async (req, res) => {
    try {
        ...
        if (!email || !password) {
            return res.status(400).json({ error: 'All details including file are required' });
        }
        ...
        const token = JWT.sign({ id: existingSupporter._id }, process.env.JWT_SECRET, { expiresIn: '7d' });
...
const loginFounderController = async (req, res) => {
    try {
        ...
        const token = JWT.sign({ id: existingFounder._id }, process.env.JWT_SECRET, { expiresIn: '7d' });
        res.cookie("token", token, {
            httpOnly: true,
            secure: false,
            sameSite: "strict",
...
const loginInvestorController = async (req, res) => {
    try {
        ...
        const token = JWT.sign({ id: existingInvestor._id }, process.env.JWT_SECRET, { expiresIn: '7d' });
        res.cookie("token", token, {
            httpOnly: true,
            secure: false,
            sameSite: "strict",
...
const SignupUserController = async (req, res) => {
    try {
        ...
        const existingSupporter = await Supporter.findOne({ email });
        if (existingSupporter) {
            return res.status(409).json({ error: 'Founder with this email already exists' });
        }
...
const registerFounderController = async (req, res) => {
    try {
        ...
        if (!file || !companyName || !ownerName || !email || !password || !aadhaar || !pancard || !linkedin || !proof || !answer) {
            return res.status(400).json({ error: 'All details including file are required' });
        }
...
const registerInvestorController = async (req, res) => {
    try {
        const { investorName, email, password, pastInvestment, linkedin, aadhaar, pancard, answer } = req.body;
        const file = req.file;
        if (!file || !investorName || !email || !password || !pastInvestment || !linkedin || !aadhaar || !pancard || !answer) {
        ...
        const newInvestor = new Investor({
            investorName, email, password, pastInvestment, linkedin, aadhaar, pancard, file: file.path, answer
        })
