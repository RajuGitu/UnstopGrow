...
import { Calendar, Heart, MessageSquare, Tag, Image, UserPlus } from "lucide-react";
...
const PostCard = ({ postItem, onComment }) => {
  ...
  const handleLike = async (postId) => {
    ...
    try {
      ...
      if (currentlyLiked) {
        ...
        // Like the post
        response = await axiosInstance.post('/supporter/supporterLikesPosts', {
          postId: postId
        }, {
          headers: {
            "Content-Type": "application/json",
  ...
  const handleFollow = async (startupId) => {
    ...
    try {
      ...
      if (currentlyFollowed) {
        ...
        // Follow the startup
        response = await axiosInstance.post('/supporter/supporterFollowPosts', {
          startupId: startupId
        }, {
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.data.success) {
        ...
        // Handle errors
        console.error('Error with startup follow/unfollow:', response.data.error);
  ...
  const isFollowed = followedStartups[postItem.startupId] ?? postItem.isFollowed;
    ...
    <Card className="bg-white/90 backdrop-blur-sm hover:shadow-lg transition-shadow duration-300 overflow-hidden">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          ...
          >
            <UserPlus className={`h-4 w-4 mr-1 ${isFollowed ? "fill-current" : ""}`} />
            {isFollowLoading ? "Processing..." : isFollowed ? "Following" : "Follow"}
          </Button>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2 pt-2">
          ...
          >
            ...
            {isLikeLoading ? "Processing..." : isLiked ? "Liked" : "Like"}
          </Button>
          <Button
            variant="outline"
            size="sm"
            className="flex-1 text-slate-600 border-slate-200 hover:bg-slate-50"
            onClick={() => onComment && onComment(postItem._id)}
          >
